...
app.post('/api/tenancies/:id/renew', async (req, res) => {
  ...
  try {
    ...
    const origResult = await client.query(
      `SELECT t.application_id, t.renter_id, COALESCE(t.owner_id, p.owner_id) as owner_id, t.property_id, t.rent_amount, t.lease_end_date, t.payment_status
       FROM tenancies t
       LEFT JOIN properties p ON t.property_id = p.property_id
       WHERE t.tenancy_id = $1 FOR UPDATE OF t`,
      [id],
    );
